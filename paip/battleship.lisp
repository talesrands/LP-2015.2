...
(defun sink-test (attack)
  (let ((ships (remove-if #'(lambda (list) (subsetp list (cons attack (cdr *board*))))
	...
	(let ((n (length (first *board*))))
	  (print `(You sank a ship! Now there is ,(- n 1) left))))
    ships))
...
(defun attack-result (attack)
  (if (null attack)
      '(Remember to only use one letter and one number)
...
(defun translate-attack (target)
  (if (eql '2 (length (symbol-name target)))
...
(defun battleship ()
  ...
  (block out
    (loop
       (print 'Your-move>)
       (let* ((attack (read)) (answer (attack-result (translate-attack attack))))
	 (if (member 'exit (list attack))
	     (return-from out))
	 (format t "~{~A ~}" answer)))))
